                    ...
                    @foreach (session('cart') as $id => $details)
                        <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                            ...
                            <div class="flex justify-center w-1/5">
                                <input type="number" class="text-center w-16 border border-gray-300 p-2 text-black"
                                    value="{{ $details['quantity'] }}" min="1" max="999"
                                    data-id="{{ $id }}">
                            </div>

                            <span class="text-center w-1/5 font-semibold text-sm text-black total-price">
                                Rp{{ $details['price'] * $details['quantity'] }}.000
                            </span>

                            <span class="text-center w-1/5 font-semibold text-sm text-black">
                                Rp{{ $details['price'] }}.000
                            </span>
                ...
                <div class="border-t mt-8">
                    ...
                    <button
                        class="bg-indigo-500 font-semibold hover:bg-indigo-600 py-3 text-sm text-white uppercase w-full"><a
                            href="{{ route('checkout.makanan') }}">Checkout</a></button>
                    <a href="#" class="flex font-semibold text-indigo-600 text-sm mt-10 update-cart"
                        data-id="{{ $id }}" data-quantity="{{ $details['quantity'] }}">
                        Update Cart
                    </a>
    ...
    <script type="text/javascript">
        $(".delete-product").click(function(e) {
            ...
            if (confirm("Do you really want to delete?")) {
                $.ajax({
                    url: '{{ route('delete.cart.product.makanan') }}',
                    method: "DELETE",
                    data: {
                        _token: '{{ csrf_token() }}',
        ...
        $("input[type=number]").on('change', function(e) {
            ...
            $.ajax({
                url: '{{ route('update.cart.product.makanan', ['id' => $id]) }}',
                method: "PATCH",
                data: {
                    _token: '{{ csrf_token() }}',
                ...
                success: function(response) {
                    ...
                    var totalPriceElement = $(".total-price[data-id=" + id + "]");
                    var newTotalPrice = response.price * newQuantity;
                    totalPriceElement.text('Rp' + newTotalPrice + '.000');
                }
            });
        });
    </script>
